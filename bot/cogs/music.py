...
class Music(commands.Cog, wavelink.WavelinkMixin):
    ...
    async def hist_command(self,ctx,show:t.Optional[int] = 10):
        player = self.get_player(ctx)
        ...
        embed = discord.Embed(
            title="History",
            description=f"Showing up to previous {show} tracks",
            colour=ctx.author.colour,
            timestamp=dt.datetime.utcnow()
        )
        embed.set_author(name="")
    ...
    @hist_command.error
    async def hist_command_error(self, ctx, exc):
        if isinstance(exc, QueueIsEmpty):
            await ctx.send("The queue is currently empty.")
    ...
    async def skipto_command(self, ctx, index: int):
        ...
        if not 0 <= player.queue.position + index <= player.queue.length:
            raise NoMoreTracks
